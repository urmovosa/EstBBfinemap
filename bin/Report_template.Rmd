...
Below is the report about SuSiE fine-mapping results over all phenotypes and loci. Each locus was defined as +/-1Mb from index SNP (in case of multiple perfect proxies with equal significance, select the first one). For each effect with identified credible sets, we visualise three plots: regional plot where color of the SNP corresponds to LD R2 with index SNP (from input dataset), regional plot where color of the SNP corresponds to credible set status (different credible sets depicted with different colors), and regional plot where on the y-axis is posterior inclusion probability of each SNP (again, color depicts credible set status).
